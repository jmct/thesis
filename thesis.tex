...
\nomenclature{$\top$}{The top element of a lattice}
    ...
    \end{chapquote}
    \todoinline{Use the following SPJ quote from Hammond's book: ``The always dubious ``feed in an arbitrary and watch it run faster'' story is comprehensively dead.''}
...
\label{part:static}
    ...
    can safely be executed in parallel. A strictness analyser can be used for
    this purpose.
    \end{chapquote}
...
\label{part:implementation}
    ...
    This can raise serious resource-management problems, since during
    evaluation a graph often expands before it shrinks.
    \end{chapquote}
    ...
    like to have an analyser that \emph{automatically} marks expressions that
    can safely be executed in parallel. A strictness analyser can be used for
    this purpose. But one often also needs to know whether parallel evaluation
    is \emph{worthwhile}.
    \end{chapquote}
